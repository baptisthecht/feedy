...
const Day = ({day, number, isToday}: {day: string, number: number, isToday: boolean}) => {
    return <div className={cn("flex flex-col  items-center gap-1 rounded-lg py-2 px-1 flex-1",{
        "bg-primary-base": isToday
    })}>    
        <p className={cn("text-paragraph-xs text-text-sub-600",{
            "text-static-white": isToday
        })}>{day.padStart(2, "0")}</p>
        <p className={cn("text-label-md text-text-strong-950",{
            "text-static-white": isToday
        })}>{number}</p>
    </div>
}